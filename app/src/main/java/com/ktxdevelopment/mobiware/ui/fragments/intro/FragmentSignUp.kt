...
class FragmentSignUp : BaseFragment(), SelectionAdapter.OnMobileClickListener {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        var i = 0
        binding.tvSignUseless.setOnClickListener {
            if (i%2 != 0) showErrorSnackbar("JUST ERROR")
            else showSuccessSnackbar("GREAT SUCCESS")
            i += 1
        }
    ...
    private fun signButtonClickListener() {
        ...
        if (TextInputClient.validateSignUpInput(
                binding.etUsernameSignIn,
                binding.etPasswordSignIn,
                binding.etEmailSignIn
            )
        ) {
    ...
    override fun onMobileClick(position: Int) {
        ...
        if (binding.tvSignPhoneModel.visibility == View.GONE) binding.tvSignPhoneModel.visibility =
            View.VISIBLE
        ...
        restViewModel.getResponse.observe(this) { res ->
            when (res) {
                is Resource.Success -> {
                    ...
                    roomViewModel.writeUserToPreferences(requireContext(), BaseClient.convertFireToLocalUser(user))
