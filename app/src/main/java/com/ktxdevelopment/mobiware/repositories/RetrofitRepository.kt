...
//    suspend fun getMobile(url: String) = api.getMobile(url)
    ...
    suspend fun searchMobile(query: String) : Resource<SearchResponse>
            = safeApiCall { api.searchMobile(query) }


        suspend fun getMobile(url: String) : Resource<GetResponse>
        = safeApiCall { api.getMobile(url) }
